...
class TestArchiveOldHistory(TestCase):
    command_name = 'archive_old_historical_records'

    def test_archive_older_than_days(self):
        out = StringIO()
        poll = Poll.objects.create(question="test", pub_date=datetime.now())
        created_history = poll.history.last()
        created_history.history_date = datetime.now() - timedelta(days=30)
        ...
        self.assertEqual(ArchivedHistoricalRecord.objects.count(), 1)

    def test_archive_older_than_weeks(self):
        out = StringIO()
        poll = Poll.objects.create(question="test", pub_date=datetime.now())
        created_history = poll.history.last()
        created_history.history_date = datetime.now() - timedelta(days=8)
